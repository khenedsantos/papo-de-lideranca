(function () {
  ...
  function renderBook() {
    ...
    const book = normalizeBook(detail.book || {});
    detail.book = book;
  ...
  function renderCover(book) {
    ...
    if (coverUrl) {
      cover.innerHTML = `<img class="book-cover-image" src="${escapeHtml(coverUrl)}" alt="${escapeHtml(alt)}" loading="eager" decoding="async">`;
      cover.classList.add("has-real-cover");
      cover.removeAttribute("aria-label");
      cover.removeAttribute("aria-hidden");
      const image = cover.querySelector(".book-cover-image");
      if (image) {
        image.addEventListener("error", () => renderCoverFallback(cover, book, alt), { once: true });
        image.addEventListener("load", () => {
          if (image.naturalWidth <= 1 || image.naturalHeight <= 1) {
            renderCoverFallback(cover, book, alt);
          }
        }, { once: true });
      }
      return;
  ...
  function renderCoverFallback(cover, book, alt) {
